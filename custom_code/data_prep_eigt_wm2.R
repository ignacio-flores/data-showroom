...
keep_eigt_wm2_rows <- function(data) {
  visible_kinship <- data$kinship %in% eigt_visible_kinship_choices
  revenue_general_government <- data$d4_concept_lab %in% revenue_concepts &
    data$kinship == "General government level"

  data[visible_kinship | revenue_general_government, , drop = FALSE]
}
