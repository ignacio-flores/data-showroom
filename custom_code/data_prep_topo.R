...
build_topo_data <- function(debt_negative = TRUE) {
  ...
  #create database with % of NI
  natinc <- read.csv("data/supplementary_var_long.csv") %>%
    filter(variable == "mnninc") %>%
    select(country, year, value) %>%
    rename(pop = `value`, GEO = `country`) %>%
    mutate(pop_lab = "Ratio to National Income")

  # data_ni <- data_nom_lcu %>%
  #   left_join(natinc, by = c("GEO", "year"), multiple = "all") %>%
  #   mutate(value = value / natinc)
  ...
  xrates_mer <- read.csv("data/supplementary_var_long.csv") %>%
    ...
    rename(`GEO` = country, `xrate` = value, `xrate_lab` = variable)
  ...
  data_rea_x <- left_join(select(data_rea, -c("xrate", "xrate_lab")), xrates_all, by = c("GEO")) %>%
    arrange(legend, d2_sector_lab, GEO, year) %>%
    mutate(value = value / xrate)
...
qs::qsave(build_topo_data(debt_negative = TRUE), "data/topo_ready.qs", preset = "fast")
qs::qsave(build_topo_data(debt_negative = FALSE), "data/topo_ready_dp.qs", preset = "fast")
