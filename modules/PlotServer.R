  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$valuePlot <- renderPlotly({
      ...
      if (!is.null(extra_layer)){
        extra_df <- df[df[[color_var]] %in% extra_layer$values, ]
        df <- df[df[[color_var]] != "Net Wealth", ]
      }
        ...
        plots <- lapply(facet_levels, function(facet_level) {
          ...
          if (!is.null(extra_layer)) {
            ...
            if (nrow(df_extra_facet) > 0) {
              plt <- plt %>%
                add_trace(
                  x = ~df_extra_facet[[x_var]],
                  y = ~df_extra_facet[[y_var]],
                  type = 'scatter',
                  mode = extra_layer$type,  # Style defined in YAML
